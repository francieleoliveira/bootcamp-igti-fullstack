...
function renderFoundUsers(users) {
   ...
   if (users.length === 0) {
      searchText.textContent = 'Resultados aparecerão aqui';
   } else {
...
function renderFoundStats(users) {
   ...
   if (users.length === 0) {
      ...
      searchText.textContent = 'Sem status disponíveis para o resultado';
      ...
      statsFemaleUsers.textContent = `Mulheres: ${femaleUsers}`;
      ...
      statsMaleUsers.textContent = `Homens: ${maleUsers}`;
